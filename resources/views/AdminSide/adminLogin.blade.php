...
<body class="color-background6">
    <div class="container-custom">
        <form action="{{ route('authenticate') }}" method="post" class="p-5">
            @csrf
            <h2 class="text-center text-uppercase font-heading color-3">Admin Login</h2>
            <div class="mb-3">
                <label for="username" class="mt-3 mb-3 font-heading">Username</label>
                <input type="text" class="form-control @error('username') is-invalid @enderror font-paragraph" id="username" name="username" placeholder="admin123..." required style="width: 400px;">
                @error('username')
